...
describe('Memory', () => {
  ...
  test('Search memories by embedding similarity', async () => {
    ...
    if (!embedding) {
      writeCachedEmbedding(similarMemoryContent, similarMemory.embedding as number[])
    }
    ...
    if (!embedding) {
      writeCachedEmbedding(dissimilarMemoryContent, dissimilarMemory.embedding as number[])
    }
  ...
  test('Verify memory similarity and ranking', async () => {
    ...
    if (!embedding) {
      writeCachedEmbedding(queryMemoryContent, queryMemory.embedding as number[])
    }
    ...
    if (!embedding) {
      writeCachedEmbedding(highSimilarityContent, highSimilarityMemory.embedding as number[])
    }
    ...
    if (!embedding) {
      writeCachedEmbedding(lowSimilarityContent, lowSimilarityMemory.embedding as number[])
    }
...
describe('Memory - Basic tests', () => {
  ...
  test('Memory lifecycle: create, search, count, and remove', async () => {
    ...
    if (!embedding) {
      writeCachedEmbedding((testMemory.content as Content).content as string, testMemory.embedding as number[])
    }
...
describe('Memory - Extended Tests', () => {
  ...
  test('Test unique insert', async () => {
    ...
    const allCount = await memoryManager.countMemoriesByUserIds([user?.id as UUID, zeroUuid], false)
    const uniqueCount = await memoryManager.countMemoriesByUserIds([user?.id as UUID, zeroUuid], true)
