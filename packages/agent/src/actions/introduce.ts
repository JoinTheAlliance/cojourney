...
export const getRelevantRelationships = async (
  ...
  const userData = await runtime.databaseAdapter.getAccountById(userId);
  ...
  searchEmbeddings.push(searchEmbedding);

  const otherPeopleDescriptions = [];
  for (let i = 0; i < searchEmbeddings.length; i++) {
    const otherPeopleDescriptionsSub = (
      ...
      (d: Memory) =>
        d.user_id !== message.senderId && d.user_id !== message.agentId
    );
    otherPeopleDescriptions.push(...otherPeopleDescriptionsSub);
  }

  //clear duplicates
  const uniqueOtherPeopleDescriptions = Array.from(
    new Set(otherPeopleDescriptions)
  );
  ...
  const userIds = Array.from(
    new Set(uniqueOtherPeopleDescriptions.map((d: Memory) => d.user_id as UUID))
  );
